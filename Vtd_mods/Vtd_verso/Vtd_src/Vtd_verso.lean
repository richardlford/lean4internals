-- lean4internals/Vtd_mods/Vtd_verso/Vtd_src/Vtd_verso.lean

        
-- Imports for contained files or directories.
import «Vtd_mods».«Vtd_verso».«Vtd_src».«Vtd_verso».«Vtd_Verso.lean»
import «Vtd_mods».«Vtd_verso».«Vtd_src».«Vtd_verso».«Vtd_Verso»
-- End of Imports



import VersoManual
import VersoExts
open Verso.Genre Manual
open Verso.Genre.Manual.InlineLean

#doc (Manual) "`m/verso/src/verso/`"  =>

%%%
authors := ["Richard L Ford"]
tag := "mods-verso-src-verso"
%%%

{editlink "Vtd_mods/Vtd_verso/Vtd_src/Vtd_verso.lean"}[edit]

TODO

{include 1 «Vtd_mods».«Vtd_verso».«Vtd_src».«Vtd_verso».«Vtd_Verso.lean»}
{include 1 «Vtd_mods».«Vtd_verso».«Vtd_src».«Vtd_verso».«Vtd_Verso»}
